...
pub(crate) fn create_window<R: Runtime>(
    ...
    if config.hide_titlebar {
        #[cfg(target_os = "macos")]
        {
            ...
            let use_native_titlebar = handle
                .state::<crate::mac_window::PluginState>()
                .use_native_titlebar();
            if !use_native_titlebar {
                win_builder =
                    win_builder.hidden_title(true).title_bar_style(TitleBarStyle::Overlay);
            }
        }
        #[cfg(not(target_os = "macos"))]
        {
...
pub(crate) fn create_main_window(handle: &AppHandle, url: &str) -> Result<WebviewWindow, String> {
    let mut counter = 0;
    let label = loop {
...
pub(crate) fn create_child_window(
    ...
    let scale_factor = parent_window.scale_factor().map_err(|e| e.to_string())?;
